	  		...
	  		<div className="subnav container flex vertical-align">
	  			...
	  			<div>
		  			...
		  			<Link className="button left-margin"
		  				to={`pages/${this.props.page.id}/view/delete`}>
		  				{"Delete"}
		  			</Link>
		  			<Link className="button left-margin"
		  				to={`pages/${this.props.page.id}/edit`}>
		  				{"Edit"}
		  			</Link>
		  		</div>
	  		</div>
	  			...
	  			<Block loading={this.props.loading}>
	  	  		<div className="container flex vertical-align">
	  	  			...
	  	  			<Link className="button"
	  	  				to={`pages/${this.props.page.id}/new_section`}>
	  	  				{"New section"}
	  	  			</Link>
	  	  		</div>
	  		  	<div className="container">
		  		  				...
		  		  				return (
			  		  				<li key={i}
			  		  					...
			  		  					<span className="flex-0 list-buttons">
			  		  						<Link to={`pages/${this.props.page.id}/sections/${section.id}/view`}>{"View"}</Link>
			  		  						<Link to={`pages/${this.props.page.id}/sections/${section.id}/edit`}>{"Edit"}</Link>
			  		  						<Link to={`pages/${this.props.page.id}/sections/${section.id}/view/delete`}>{"Delete"}</Link>
			  		  					</span>
			  		  				</li>
			  		  			)
		  		  			})}
